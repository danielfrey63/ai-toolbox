...
- `--diarize [BACKEND]` — enable speaker diarization. Output transcript lines become `[MM:SS] [<speaker>] text`. Backends:
  ...
  - **`--diarize pyannote-local`** — local pyannote.audio. Free but heavy: requires `pip install pyannote.audio` (pulls PyTorch), a Hugging Face token (`HF_TOKEN`), and license acceptance for `pyannote/speaker-diarization-3.1` on huggingface.co. Slow on CPU; reasonable on GPU. Diarization runs locally, then aligns to the Whisper transcript.
  - Identifying *which name* maps to `A`/`B`/`SPEAKER_00` is done later by Claude using address patterns in the transcript (see Step 4 Inventar → Personen & Stimmen).
